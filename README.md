...
Linked documents can be expanded into an XML context file, matching the
[`create_ctx`](https://llmstxt.org/intro.html) / `llms_txt2ctx` behaviour of the
Python reference library. The `Optional` section is skipped by default.
...
$context = $llmsTxt->toLlmContext(); // OR ->toLlmContext(true) to include Optional
$llmsTxt->toLlmContextFile('/path/to/llms-ctx.txt');
```
